...
const AuthMidware =
   ...
   async (req: Request, res: Response, next: NextFunction) => {
      try {
         ...
         if (!token) throw new BadRequest("No access token found in request header");
         const verifiedUser = JwtUtils.decrypt<IJwtUser>(token);

         if (requiredRoles.length && !requiredRoles.includes(verifiedUser.role)) throw new Unauthorized();

         if (verifiedUser.role === EUserRoles.BLOCKED) throw new Forbidden();
