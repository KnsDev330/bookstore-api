...
const BookService = {
   ...
   getSingleBookById: async (id: string) => {
      ...
      if (!book)
         throw new BadRequest(`Book not found with given ID`);
      return book;
   },
   ...
   updateSingleBookById: async (id: string, book: Partial<IBook>) => {
      ...
      if (!updatedBook)
         throw new InternalServerError(`Could not update book, does the book exists?`);
      return updatedBook;
   },
   ...
   deleteSingleBookById: async (id: string) => {
      ...
      if (!deletedBook)
         throw new InternalServerError(`Could not delete book, does the book exists?`);
   ...
   /* get all books */
   getAllBooks: async (whereConditions: any, sortConditions: { [key: string]: SortOrder }, skip: number, limit: number) => {
